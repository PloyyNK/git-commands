...
3. (Pushing a new branch) You commit some files to the `dev-foo` branch and try to "push" them to Github, but it fails as shown here:
   ...
   ```
   Explain this error.
   > This error happened when you have not configured git in such a way that it creates a new branch on the remote. So, you are only creating a new branch on the local computer.
...
6. Consider this situation:
   ...
   >Your local repo is now not up-to-date because the change you made on Github's web interface.
   This is how to fix:
   ```
